package com.lmg.hibernate.stategy;
...
public class test {
	...
	@Test
	public void testMany2OneStategy(){	
...
//		System.out.println(order.getCustomer().getCustomerName());
		...
		//1. lazy 取值为 proxy 和 false 分别代表对应的属性采用延迟检索和立即检索
	...
	public void testSetFetch(){	
		...
		//set 集合的 fetch 属性:
		//1. 默认的 select. 通过正常的方式来初始化 set 元素
		//2. 可以取值为 subselect. 通过子查询的方式来初始化所以的 set 集合. 子查询
		//作为 where 子句的 in 的条件出现, 子查询查询所以 1 的一端的 ID. 此时 lazy 有效.
		//但 batch-size 失效.
		//3. 若取值为 join. 则
	...
	@Test
	public void testOne2ManyLevelStategy(){	
		Customer customer = (Customer) session.get(Customer.class, 1);
	...
	@Test
	public void testClassLevelStategy(){	
